...
class Util
{
    ...
    const AREA = [
        'Derecho aplicado a las Nuevas Tecnologías',
        'Derecho mercantil y societario',
        'Gestión de equipos, gestión del capital humano y personas',
        'Metodología Lean Startup, Propuesta de Valor y Prototipado',
        'Financiación de startups y capital-riesgo',
        'Finanzas corporativas',
        'Modelos y Planes de Negocio',
        'Estrategias y planes comerciales',
        'Estrategia de desarrollo de clientes',
        'Marketing digital, Comunicación y Relaciones Públicas',
        'Visibilidad y posicionamiento web',
        'Desarrollo de productos digitales',
        'Estrategia Empresarial',
        'Operaciones y Gobierno Corporativo',
     ...
     * @return string
     */
    public static function human_time_diff($from, $to = '', $as_text = false, $suffix = ' ')
    {
        if ($to == '') {
            $to = time();
        }

        $from = UTCDateTime::create(date('Y-m-d H:i:s', $from));
        $to   = UTCDateTime::create(date('Y-m-d H:i:s', $to));
        $diff = $from->diff($to);
     ...
     * @return string
     */
    public static function human_time_to_string($to = '', $as_text = false, $suffix = ' ')
    {

        $from = UTCDateTime::create(date('d-m-Y H:i:s', -3600));
        $to   = UTCDateTime::create(date('d-m-Y H:i:s', $to));
        $diff = $from->diff($to);
    ...
    public static function daysBetween(\DateTime $dateFrom, \DateTime $dateTo, $onlyWorkingDays = false){
        ...
        for ($date = $dateFrom->format('d-m-Y') ; $date <= $dateTo->format('d-m-Y') ; $date = date("d-m-Y", strtotime($date ."+ 1 days"))){
            ...
            $daysArray[$date]['from'] = \DateTime::createFromFormat('d-m-Y', $date)->setTime(0,0,0);
            $daysArray[$date]['to'] = \DateTime::createFromFormat('d-m-Y', $date)->setTime(23,59,59);
        }
    ...
    public static function weeksBetween(\DateTime $dateFrom, \DateTime $dateTo){
        ...
        for ($date = $dateFrom->format('d-m-Y') ; $date <= $dateTo->format('d-m-Y') ; $date = date("d-m-Y", strtotime($date ."+ 1 weeks"))){

            // Lunes de la semana
            $mondayOfWeek   = date("d-m-Y", strtotime('Monday this week '.$date));
            // Domingo de la semana
            $sundayOfWeek   = date("d-m-Y", strtotime('Sunday this week '.$date));

            // Primera fecha
            if($date == $dateFrom->format('d-m-Y')){
                $dateFromSelected = $dateFrom->format('d-m-Y');
            } else {
                $dateFromSelected = $mondayOfWeek;
            }

            if($sundayOfWeek <= $dateTo->format('d-m-Y')){
                $dateToSelected = $sundayOfWeek;
            } else {
                $dateToSelected = $dateTo->format('d-m-Y');
            }



            $daysArray[$date]['from']   = \DateTime::createFromFormat('d-m-Y', $dateFromSelected)->setTime(0,0,0);
            $daysArray[$date]['to']     = \DateTime::createFromFormat('d-m-Y', $dateToSelected)->setTime(23,59,59);
        }
    ...
    public static function monthsBetween(\DateTime $dateFrom, \DateTime $dateTo){
        ...
        for ($date = $dateFrom->format('d-m-Y') ; $date < $dateTo->format('d-m-Y') ; $date = date("d-m-Y", strtotime($date ."+ 1 months"))){

            // primer día del mes
            $firstOfMonth   = date("d-m-Y", strtotime('first day of this month '.$date));
            // último dia del mes
            $lastOfMonth   = date("d-m-Y", strtotime('last day of this month '.$date));
            ...
            if($lastOfMonth <= $dateTo->format('d-m-Y')){
    ...
    public static function yearsBetween(\DateTime $dateFrom, \DateTime $dateTo){
        ...
        for ($date = $dateFrom->format('d-m-Y') ; $date < $dateTo->format('d-m-Y') ; $date = date("d-m-Y", strtotime($date ."+ 1 years"))){

            // primer día del mes
            $firstOfYear   = date("d-m-Y", strtotime('first day of January '.$date));
            // último dia del mes
            $lastOfYear   = date("d-m-Y", strtotime('last day of December '.$date));
            ...
            if($lastOfYear <= $dateTo->format('d-m-Y')){
    ...
    public static function endMatchDayDate($startDate, array $matchDays){
        ...
        $start = UTCDateTime::create($startDate);
     ...
     * Calcula la fecha de fin de Jornada
     *
     * @param float $seconds Tiempo en segundos
     * @param bool $short Si es true, muestra la cadena de salida abreviada
     *
     * @return \DateTime|boolean
     */
    public static function getLiteralFromSeconds(float $seconds, bool $short=false) :string
    {
        $time = self::conversorSegundosHoras($seconds);

        $hours = $time["hours"];
        $mins = $time["min"];
        $secs = $time["sec"];

        $hourString = $short ? 'h':'horas';
        $minString = $short ? 'm':'minutos';
        $secString = $short ? 's':'segundos';

        if($hours >0)
        {
            return $hours." ".$hourString .", ". $mins." ".$minString." y ".round($secs)." ".$secString;
        }
        elseif($mins <= 0){
            return round($secs)." ".$secString;
        }
        else{
            return $mins." ".$minString." y ".round($secs)." ".$secString;
        }

    }


    public static function conversorSegundosHoras($tiempo_en_segundos):array {
    ...
    public static function calculateEstimatedDate($totalMatchs, $startDate, $endDate, $matchIndex) {
        ...
        $startDate  = UTCDateTime::create($startDate);
        $endDate    = UTCDateTime::create($endDate);
    ...
    /**
     * Obtiene el objeto datetime de un string de fecha del tipo d-m-Y H:i:s - d-m-Y H:i:s
     *
     * @param $type
     * @param $string
     *
    ...
    public static function getRangeDate($type, $string){
        ...
        switch ($type){
            case "from":
                $date = \DateTime::createFromFormat('d-m-Y H:i:s', $extract[0]);
                if(!$date){
                ...
                break;
            case "to":
                $date = \DateTime::createFromFormat('d-m-Y H:i:s', $extract[1]);
                if(!$date){
    ...
    public static function nextMatchDay(\DateTime $from, \DateTime $to){
        ...
        $nextFrom = \DateTime::createFromFormat('d-m-Y H:i:s', $startToNextMatchDay->format('d-m-Y H:i:s'));
    ...
    public static function getMatchHoursInterval(\DateTime $start_date, array $round_hours){
        ...
        foreach ($round_hours as $index => $round_hour){
            $resultRounds[] = [
                "diff_start" => \DateTime::createFromFormat('d-m-Y H:i:s',$round_hour['start_date'])->getTimestamp() - $dateTimestamp,
                "diff_end" => @$round_hour['end_date'] ? \DateTime::createFromFormat('d-m-Y H:i:s',$round_hour['end_date'])->getTimestamp() - $dateTimestamp : null,
            ];
        }
    ...
    public static function addSecondsToDateTime(\DateTime $startDate, $seconds) {
        ...
        $date = \DateTime::createFromFormat('d-m-Y H:i:s', $startDate->format('d-m-Y H:i:s'));
    ...
    public static function getDateFromLiteral($rangeSelected = 'today', $type = 'from') {
        $from = (UTCDateTime::create('now'))->setTime(0,0,0);
        $to = (UTCDateTime::create('now'))->setTime(23,59,59);
        if($rangeSelected === 'this_month') {
            $from = (UTCDateTime::create('first day of this month'))->setTime(0,0,0);
            $to = (UTCDateTime::create('last day of this month'))->setTime(23,59,59);
        } elseif ($rangeSelected === 'last_month') {
            $from = (UTCDateTime::create('first day of last month'))->setTime(0,0,0);
            $to = (UTCDateTime::create('last day of last month'))->setTime(23,59,59);
        }elseif ($rangeSelected === 'this_week') {
            $from = (UTCDateTime::create('monday this week'))->setTime(0,0,0);
            $to = (UTCDateTime::create('sunday this week'))->setTime(23,59,59);
        }
    ...
    public static function getDatesFromOption($option, $dates = null, $recovery = "from")
    {
        ...
        if($option == "month")
        {
            $firstDay = (UTCDateTime::create('first day of this month'))->setTime(0,0,0);
            $lastDay = (UTCDateTime::create('now'))->setTime(23,59,59);
        }

        if($option == "week")
        {
            $firstDay = (UTCDateTime::create('monday this week'))->setTime(0,0,0);
            $lastDay = (UTCDateTime::create('now'))->setTime(23,59,59);
        }

        if($option == "today")
        {
            $firstDay = (UTCDateTime::create('now'))->setTime(0,0,0);
            $lastDay = (UTCDateTime::create('now'))->setTime(23,59,59);
        }
    ...
    public static function splitNames(string $full_name, bool $apellidoPrimero)
    {
        if($apellidoPrimero == true) {
            /* separar el nombre completo en espacios */
            //$tokens = explode(" ",str_replace(PHP_EOL,"",$full_name));
            ...
            switch ($num_nombres) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    $primerApellido = $names[0];
                    $nombres = $names[1];
                    break;
                case 3:
                    $primerApellido = $names[0];
                    $segundoApellido = $names[1];
                    $nombres = $names[2];
                    break;
                case 4:
                    $primerApellido = $names[0];
            ...
            return ["surname1" => $primerApellido, "surname2" => $segundoApellido, "name" => $nombres];
        }
        else{
            /* separar el nombre completo en espacios */
            $tokens = explode(' ', trim($full_name));
            ...
            switch ($num_nombres) {
                ...
                default:
                    ...
                    $primerApellido = $names[2];
                    $segundoApellido = $names[3];
    ...
    public static function getTelephoneNumber(string $telephone): string {
        return substr($telephone, 2);
    }

    public static function getTelephonePrefix(string $telephone): string {
